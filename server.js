      ...
      if(cash["trst"]===1){
        if(message.channel.id===cash["msgch"]){
          ...
          try{
              ...
              var enres = await fetch(`https://script.google.com/macros/s/AKfycbx2zxXArFJuPDctM7zrFEz73kVI6Y8JUcpr_GkxnyZeJT4c4mx8rSTSL-dqD4x7fEed/exec?text=${trtext}&source=&target=${encodeURIComponent("en")}`).then(res => res.text())
    if(jares==="[リンク省略]"){
      return
...
client.on('messageUpdate',async (oldMessage,newMessage) => { //メッセージが編集されたことを検知
  if(trmsgid[oldMessage.id]===undefined){ //キャッシュにメッセージidなかったら除外
    ...
    var enres = await fetch(`https://script.google.com/macros/s/AKfycbx2zxXArFJuPDctM7zrFEz73kVI6Y8JUcpr_GkxnyZeJT4c4mx8rSTSL-dqD4x7fEed/exec?text=${newMessage.content}&source=&target=${encodeURIComponent("en")}`).then(res => res.text())
    if(jares==="[リンク省略]"){
      return
    } 
